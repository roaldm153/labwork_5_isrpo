...
TEST(ArgParserTestSuite, HelpStringTest) {
    ...
    //     "without short argument, --number=<int>, Some Number\n"
...
TEST(ArgParserTestSuite, NegativePositionalFlagTest) {
    ArgParser parser("My Parser");
    parser.AddIntArgument('p', "param1");
    parser.AddIntArgument("param2").MultiValue().Positional();
    parser.AddFlag('f', "flag1");
    parser.AddFlag('a', "flag2");
    parser.AddFlag('b', "flag3");
    parser.AddFlag('c', "flag4");
    parser.Parse(SplitString("app -p -1 -2 -fabc -3 -4 -5"));

    ASSERT_TRUE(parser.GetFlag("flag1"));
    ASSERT_TRUE(parser.GetFlag("flag2"));
    ASSERT_TRUE(parser.GetFlag("flag3"));
    ASSERT_TRUE(parser.GetFlag("flag4"));
    ASSERT_EQ(parser.GetIntValue("param1"), -1);
    ASSERT_EQ(parser.GetIntValue("param2", 0), -2);
    ASSERT_EQ(parser.GetIntValue("param2", 1), -3);
    ASSERT_EQ(parser.GetIntValue("param2", 2), -4);
    ASSERT_EQ(parser.GetIntValue("param2", 3), -5);
}

TEST(ArgParserTestSuite, VariableArgumentsTest) {
    ArgParser parser("My Parser");
    std::vector<int32_t> result;
    parser.AddIntArgument('a', "param1").MultiValue().StoreValues(result);

    ASSERT_TRUE(parser.Parse(SplitString("app -a 1 -a=2 --param1 4 --param1=8")));
...
TEST(ArgParserTestSuite, FiveFlagsTest) {
    ArgParser parser("My Parser");
    parser.AddFlag('a', "param1");
    parser.AddFlag('b', "param2");
    parser.AddFlag('c', "param3");
    parser.AddFlag('d', "param4");
    parser.AddFlag('e', "param5");

    ASSERT_TRUE(parser.Parse(SplitString("app -abcde")));
    ASSERT_TRUE(parser.GetFlag("param1"));
    ASSERT_TRUE(parser.GetFlag("param2"));
    ASSERT_TRUE(parser.GetFlag("param3"));
    ASSERT_TRUE(parser.GetFlag("param4"));
    ASSERT_TRUE(parser.GetFlag("param5"));
}